...
namespace TdgMvc
{
    public class GeneratorLevelController
    {
        ...
        private MarchingSquaresGeneratorLevel _marchingSquaresGeneratorLevel = new
            MarchingSquaresGeneratorLevel();
         ...
         public void Awake()
        {
         ...
         private void RandomFillLevel()
         {
             ...
             for (var x = 0; x < _widthMap; x++)
             {
                 ...
                 {
                     if (x == 0 || x == _widthMap - 1 || y == 0 || y == _heightMap
                         - 1)
                         _map[x, y] = 1;
                     else
                         _map[x, y] = (pseudoRandom.Next(0, 100) <
                                       _randomFillPercent) ? 1 : 0;
                 }
             }
         }
         ...
         {
             var wallCount = 0;
             for (var neighbourX = gridX - 1; neighbourX <= gridX + 1;
                 neighbourX++)
             {
                 for (var neighbourY = gridY - 1; neighbourY <= gridY + 1;
                     neighbourY++)
                 {
                     if (neighbourX >= 0 && neighbourX < _widthMap && neighbourY
                         >= 0 && neighbourY < _heightMap)
                     {
